...
impl Mfp68901 {
    ...
    pub fn reset(&mut self, host_replay_rate: u32) {
        ...
        self.timers[TimerName::TimerC as usize].enable = true;
        self.timers[TimerName::TimerC as usize].mask = true;

        // gpi7 is not really a timer, "simulate" an event type timer with count=1 to make the code simpler
        self.timers[TimerName::Gpi7 as usize].control_register = 1 << 3; // simulate event mode
        self.timers[TimerName::Gpi7 as usize].data_register_init = 1; // event count always 1
        self.timers[TimerName::Gpi7 as usize].data_register = 1;

        self.host_replay_rate = host_replay_rate;
    }

    pub fn write8(&mut self, port: usize, data: u8) {
        let port = port & 255;
        debug_assert!((port & 1) == 1);

        if (port & 1) != 0 {
            match port {
                0x19 => {
                    self.timers[TimerName::TimerA as usize].set_cr(data & 0x0f);
                }
                0x1b => {
                    self.timers[TimerName::TimerB as usize].set_cr(data & 0x0f);
                }
                0x1d => {
                    self.timers[TimerName::TimerC as usize].set_cr((data >> 4) & 7);
                    self.timers[TimerName::TimerD as usize].set_cr(data & 7);
                }
                0x1f | 0x21 | 0x23 | 0x25 => {
                ...
                0x07 => {
                    self.timers[TimerName::TimerA as usize].set_er((data & (1 << 5)) != 0);
                    self.timers[TimerName::TimerB as usize].set_er((data & (1 << 0)) != 0);
                    self.timers[TimerName::Gpi7 as usize].set_er((data & (1 << 7)) != 0);
                }
                0x09 => {
                    self.timers[TimerName::TimerC as usize].set_er((data & (1 << 5)) != 0);
                    self.timers[TimerName::TimerD as usize].set_er((data & (1 << 4)) != 0);
                }
                0x13 => {
                    self.timers[TimerName::TimerA as usize].set_mr((data & (1 << 5)) != 0);
                    self.timers[TimerName::TimerB as usize].set_mr((data & (1 << 0)) != 0);
                    self.timers[TimerName::Gpi7 as usize].set_mr((data & (1 << 7)) != 0);
                }
                0x15 => {
                    self.timers[TimerName::TimerC as usize].set_mr((data & (1 << 5)) != 0);
                    self.timers[TimerName::TimerD as usize].set_mr((data & (1 << 4)) != 0);
                }
                _ => {}
            }
            self.regs[port] = data;
    ...
    pub fn read8(&self, port: usize) -> u8 {
        let port = port & 255;
        let mut data = 0xff;
    ...
    pub fn read16(&self, port: usize) -> u16 {
        0xff00 | self.read8(port + 1) as u16
    }

    pub fn write16(&mut self, port: usize, data: u16) {
        self.write8(port + 1, data as u8);
    }

    pub fn tick(&mut self, timer_id: usize) -> bool {
        self.timers[timer_id].tick(self.host_replay_rate)
    }

    pub fn set_ste_dac_external_event(&mut self) {
        self.timers[TimerName::TimerA as usize].external_event = true;
        self.timers[TimerName::Gpi7 as usize].external_event = true;
    }
}

...
impl SteDac {
    ...
    pub fn write8(&mut self, ad: usize, data: u8) {
        let ad = ad & 0xff;
        if (ad & 1) != 0 {
            let mut data = data;
            match ad {
    ...
    pub fn write16(&mut self, ad: usize, data: u16) {
        let ad = ad & 0xff;
        if (ad & 1) == 0 {
            match ad {
                0x22 => {
                ...
                _ => {
                    self.write8(ad + 1, data as u8);
                }
            }
        }
    }

    pub fn read8(&self, ad: usize) -> u8 {
        let ad = ad & 0xff;
        let mut data = 0xff;
        if (ad & 1) != 0 {
            data = self.regs[ad];
            match ad {
                ...
                0x0d => data = (self.sample_ptr >> 0) as u8,
                _ => {}
            }
        }
        data
    ...
    pub fn read16(&mut self, ad: usize) -> u16 {
        let ad = ad & 0xff;
        let data = if (ad & 1) == 0 {
            match ad {
    ...
    pub fn compute_next_sample(&mut self, atari_ram: &[u8], mfp: &mut Mfp68901) -> i16 {
        if (self.regs[1] & 1) != 0 {
    ...
    fn microwire_tick(&mut self) -> u16 {
        if self.microwire_shift > 0 {
            // rol.w #1
            self.microwire_mask = (self.microwire_mask << 1) | (self.microwire_mask >> 15);
